...
// Per-client-IP token bucket: `burst` immediate requests, then one per
// `every`. In-memory and per-replica, which is fine for this scale — the point
// is stopping runaway loops, not precise global quotas.
